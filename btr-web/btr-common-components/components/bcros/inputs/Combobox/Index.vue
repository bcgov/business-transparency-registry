<template>
  ...
  >
    <template #default="{ error }">
      ...
      >
        <UButton
          ...
          :class="{ 'border-b-red-500': error }"
        >
          <span
            v-if="model && model.length === 0"
            class="w-full text-left"
            :class="`text-${ (!!error || isInErrorState) ? 'red-500' : 'gray-700' }`"
          >
            {{ labelPlaceholder }}
          </span>
    ...
    <template #help>
      <span :class="{ 'text-red-500': isInErrorState }">
        {{ help }}
      </span>
    </template>
  </UFormGroup>
...
const props = defineProps({
  ...
  isInErrorState: { type: Boolean, default: false },
